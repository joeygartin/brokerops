...
Built as a deliberate demonstration of production agent architecture: multi-workflow
LangGraph orchestration, durable human-in-the-loop, an MCP tool boundary, hexagonal
...
1. **Hexagonal core.** All domain logic lives in `core/` as plain Python + Pydantic.
   `core/` imports no LangGraph, no FastAPI, no SDK. Orchestration frameworks are
   shells around it.
...
5. **State is durable.** LangGraph's Postgres checkpointer backs all graphs. A
   workflow paused on a human approval survives restarts, deploys, and Cloud Run
...
```
 ...
 Cloud Scheduler ────▶│      ├── orchestration/langgraph   (3 graphs, checkpointer)    │
                      │      │        │ ports (Protocol)                               │
                      │      │        ├── integrations/mls_reso      (RESO Web API)    │
                      │      │        ├── integrations/followupboss  (FUB REST)        │
                      │      │        └── integrations/vapi          (Vapi REST)       │
                      │      ▼                                                         │
                      │  Cloud SQL Postgres ── domain tables + checkpoint tables       │
...
orchestration/langgraph/ # graphs/, state.py, checkpointer.py
...
draft → `interrupt()` at a human approval gate → approved drafts fan out into real
...
`/internal/cron/milestones`, which runs one graph per active transaction. The
`milestone_engine` core service owns every deadline rule (due-soon windows, severity
ordering, external-blocker override); the graph only routes on its results. Near
...
1. A node calls `interrupt(payload)` — the graph checkpoints and stops.
...
Because state lives in Postgres (checkpoints) and approvals are rows (not in-memory
futures), a pause costs nothing and survives anything short of losing the database.

This uniformity is also the portability story: swapping the orchestration framework
(e.g., to Google's ADK, which consumes MCP tools natively) replaces only
`orchestration/` — nodes contain no business logic, all external calls go through
ports, all HITL passes through `ApprovalRequest` rows, and state schemas are
...
showing_feedback. LangGraph's checkpointer manages its own tables alongside.
...
~95 tests: OData contract tests pinning the RESO subset, adapter tests against the
stubs (the same shapes the real APIs return), graph tests for every branch of all
three workflows, API-level flow tests, a Postgres restart-survival test (runs in CI
against a service container), and a scripted e2e demo check against the full compose
stack. Ruff + mypy strict across the workspace; gitleaks on every commit and in CI.
