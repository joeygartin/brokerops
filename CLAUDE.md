...
brokerops — AI-powered backoffice for real estate brokerages. LangGraph
orchestration behind a thin `WorkflowEngine` seam (ADR-0019) over an MCP integration
boundary (mock RESO Web API MLS, FollowUpBoss, Vapi), a framework-free domain core,
human-in-the-loop approvals, and per-client GCP deploys via Terraform.
...
1. `core/` is framework-free: plain Python + Pydantic only. No LangGraph or
   FastAPI imports — the orchestration framework is a shell around it.
...
4. All human-in-the-loop approvals pass through `ApprovalRequest` rows and a single
   resume endpoint.
5. No framework imports (LangGraph) in production code outside the
   `orchestration/langgraph/` package — the api depends only on the `WorkflowEngine`
   protocol (tests that exercise the engine may import it). This seam kept the
   orchestrator swappable: a second engine once ran the same workflows behind it,
   proving the port is mechanical (ADR-0019, superseding ADR-0004). The engine must
   stay green: scenario suites, the restart proof, and the e2e gate.
